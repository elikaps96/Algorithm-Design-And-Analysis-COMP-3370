...
public abstract class HashTable {
	...
	// loops through all words retrieved from scanner, strips them of
	// non-characters,
	...
	// either inserts a trigram into the table or increments its count by 1 if
	...
	// Looks in a random hash bucket to get the first two words to start the
	// babble.
	...
	public double getStDev() {
		...
		double avg = accum / ((1.0) * tableSize);

		double devAccum = 0;
		for (int i = 0; i < tableSize; i++)
			devAccum += (tmp[i] - avg) * (tmp[i] - avg);

		devAccum = devAccum / tableSize;
		return Math.sqrt(devAccum);
	}
}
